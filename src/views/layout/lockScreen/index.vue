			...
			<transition name="el-zoom-in-center">
				<div v-show="isShowLoockLogin" class="layout-lock-screen-login">
					<div class="layout-lock-screen-login-box">
						...
						<div class="layout-lock-screen-login-box-name">{{getUserInfos.account}}</div>
...
<script lang="ts">
import { nextTick, onMounted, reactive, toRefs, ref, onUnmounted, computed, getCurrentInstance } from 'vue';
...
export default {
	...
	setup() {
		...
		const onLockScreenSubmit = () => {
			if(state.lockScreenPassword !== getUserInfos.value.password){
				ElMessage.warning('密码不正确！')
				return false
			}
			...
			// 登录成功提示
			setTimeout(() => {
				ElMessage.success(`${getUserInfos.value.account}，欢迎回来！`)
			}, 300)
		};
		// 页面加载时
		onMounted(() => {
